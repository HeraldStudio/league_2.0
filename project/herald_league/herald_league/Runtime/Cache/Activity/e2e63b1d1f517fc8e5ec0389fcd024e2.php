...
var m3D = function () {
	...
	var diapo = [],
		...
		selected,
		urlInfo,
		imagesPath = "__ROOT__/Uploads/LeaguePost/Original/",
...
setTimeout(function() {
	m3D.init(
		[ 
			...
			<?php if(is_array($wall)): foreach($wall as $key=>$w): ?>{ src: '<?php echo ($w["activity_post_add"]); ?>', url: '<?php echo U('/Activity/Activity/detail/activityid/');?>/<?php echo ($w["id"]); ?>', title: '<?php echo ($w["activity_name"]); ?>', color: '#fff' },<?php endforeach; endif; ?>
