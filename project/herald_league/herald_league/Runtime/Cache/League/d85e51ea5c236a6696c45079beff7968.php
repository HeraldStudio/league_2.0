...
<?php if(is_array($street)): $i = 0; $__LIST__ = $street;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vs): $mod = ($i % 2 );++$i;?><h2><a href = "__APP__/League/Street/leaguelist/streetid/<?php echo ($vs["id"]); ?>"><?php echo ($vs["street_name"]); ?></a><br/></h2>
		<?php if(is_array($league)): foreach($league as $vlk=>$vl): if(($vlk) == $vs["id"]): if(is_array($vl)): $i = 0; $__LIST__ = $vl;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vle): $mod = ($i % 2 );++$i;?><a href = "__APP__/League/Home/clubIndex/leagueid/<?php echo ($vle["id"]); ?>"><?php echo ($vle["league_name"]); ?></a><br/><?php endforeach; endif; else: echo "" ;endif; endif; endforeach; endif; ?>
